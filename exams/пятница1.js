...
// Ответ:  setBtc(prevState => prevState + 1)
...
function getAverage(marks){
  return Math.round(marks.reduce((ac, num) => ac + num, 0) / marks.length)
}

console.log(getAverage([1,1,1,1,1,1,1,2]));

var cubeChecker = function(volume, side){
  if (volume && side && Math.sign(volume > 0) && Math.sign(side) > 0) {
